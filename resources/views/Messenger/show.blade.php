	...
	<header class="flex items-center mb-4 py-4">
		<div class="flex justify-between mt-2 items-end w-full">
			<p class="text-muted font-light">
                ...
                <a href="/messages" class="text-muted no-underline hover:underline"> Messages </a> / <b>{{ $convo->vendor->name }}</b>
            </p>
		</div>
	</header>
	...
	<main>
				...
				<div class="mb-8">
					<div class="w-full">
						<div style="float: right">
							<h2 class="text-lg font-normal mb-8" style="font-size: 3rem; margin-top: -8rem; color: orange; float: right"> <b>{{ $convo->vendor->name }}</b></h2>
						</div>
					</div>
						...
						@if ($message->sender == 'vendor')
							<div class="w-full">
								<div class="card mb-5" style="height:50%; width: 50%; margin-left: 45rem; position: relative;"> 
									<div class="flex w-full">
										<a href="{{ $message->path() }}">
											<img src="/uploads/avatars/{{ $message->vendor->avatar }}" style="width: 42px; height:42px; top:10px; left:10px; border-radius: 50%; margin-bottom: -10px; margin-right: 10px;">
										</a>
		...
		<div class ="lg:w-full mt-6">
			<div class="pt-6 mt-6">
				...
				<form action="{{ $convo->path(). '/reply' }}" method="POST">
					@csrf              
					<div class="pt-6 mb-3">
