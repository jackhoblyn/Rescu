...
@section('content')
	...
	<main>
		...
		<div class="card mx-auto" style="max-height: 90%; position: relative; margin-bottom: 50px;">
			...
			<div class="text-black pt-9 pl-6" style="width: 50%; float: left; min-width: 50%">
				<h2 style = "font-size: 0.9rem;">{{ ($ad->description) }}</h2>
			</div>

			<div class="mt-8 pt-8">
				<img class = "center" src="/uploads/photos/{{ $ad->photo }}" alt="Card image" style="width:100% max-height: 300px; max-width: 300px; padding-top: 1%">
					<div style="float: right">
						<form enctype="multipart/form-data" action="{{$ad->full()}}" method="POST">
	                    	...
	                    	<div class="field mt-3" style=" margin-right: 15rem;">
		                    	<input type="submit" class="button center" style="margin-top: .5rem; background-color: #6d34dc">
	                    	</div>
		                </form>
	                </div>
			</div>
			...
			<div class = "mt-8 pt-8 pl-8 ml-6" style="width: 5rem; position: relative">
				<a href="{{ $ad->edit() }}" class="mt-6 pt-6" style="text-decoration: none">
					<div class="flex w-full pt-3">
						<button type="submit" class="button2" style="min-width: 8rem; margin-left: -1.7rem; font-size: 0.9rem; ">Edit</button>
					</div>
				</a>
			</div>
		</div>
